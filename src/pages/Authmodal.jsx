      ...
      >
        ...
        <div style={{ backgroundColor: '#004851', padding: '28px 28px 20px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '14px', marginTop: '6px', marginBottom: 0 }}>
            {tab === 'signup' ? '👋 Create your account to get started' : '🔐 Login to your account'}
          </p>
        </div>
        ...
        <div style={{ padding: '24px 28px' }}>
          <AnimatePresence>
            {error && (
              <motion.div className="alert alert-danger py-2 small"
                initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ⚠️ {error}
              </motion.div>
            )}
            {success && (
              <motion.div className="alert alert-success py-2 small"
                initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ✅ {success}
              </motion.div>
            )}
            ...
            <motion.form onSubmit={handleSignup}
              ...
              <p className="text-center text-muted small mt-3 mb-0">
                ...
                <span style={{ color: '#004851', cursor: 'pointer', fontWeight: '700' }}
                  onClick={() => { setTab('login'); setError(''); setSuccess('') }}>Login here</span>
              </p>
            </motion.form>
          )}
          ...
          {tab === 'login' && (
            <motion.form onSubmit={handleLogin}
              ...
              <p className="text-center text-muted small mt-3 mb-0">
                ...
                <span style={{ color: '#004851', cursor: 'pointer', fontWeight: '700' }}
                  onClick={() => { setTab('signup'); setError(''); setSuccess('') }}>Sign up here</span>
              </p>
            </motion.form>
          )}
        </div>
